...
async function ensureHouseExists(user) {
  // 1. Own a house?
  const ownedSnap = await db.collection('houses')
    .where('ownerId', '==', user.uid).limit(1).get();
  if (!ownedSnap.empty) { currentHouseId = ownedSnap.docs[0].id; return; }

  // 2. Member of a shared house?
  const memberSnap = await db.collection('houses')
    .where('members', 'array-contains', user.uid).limit(1).get();
  if (!memberSnap.empty) { currentHouseId = memberSnap.docs[0].id; return; }

  // 3. Create a new house (migrate localStorage data if any)
...
async function handleAuthState(user) {
  ...
  if (user) {
    currentUser = user;
    try {
      // Save profile so others can look us up by email
      ...
      // Load Firestore data (overrides local)
      const doc = await db.collection('houses').doc(currentHouseId).get();
      if (doc.exists) {
        const d = doc.data();
        state.rooms = d.rooms || [];
        state.tasks = d.tasks || [];
      ...
      // Still load from localStorage so the app is usable
      loadStateLocal();
    }
